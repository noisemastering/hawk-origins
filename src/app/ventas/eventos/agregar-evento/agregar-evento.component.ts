...
export class AgregarEventoComponent implements OnInit {
  ...
  constructor(
    ...
    public dRef: MatDialogRef<AgregarEventoComponent> // Adaptar al objeto correspondiente
  ) { }
  ...
  duplicate(valor: string){ // Adaptar a los campos utilizados
    ...
    switch(valor){
      case 'nombre':
        this.duplicateService.checkForDuplicates("duplicate", "Descripcion", "eventoes", this.theForm.controls['descripcion'].value, "Descripcion")
      ...
      break;
      case 'locacion':
        this.duplicateService.checkForDuplicates("duplicate", "RazonSocial", "eventoes", this.theForm.controls['razon'].value, "RazonSocial")
        .subscribe((response: SimpleResponse) => {
          if(response.value=="Duplicate"){
  ...
  addRecord(): void {
    ...
    this.evento.ID= '';
    this.evento.Descripcion=  this.theForm.controls['descripcion'].value;
    this.evento.Notas=  this.theForm.controls['notas'].value;
    this.evento.Keywords=  JSON.stringify(this.keywords);
    this.evento.uaFecha=  "";
    this.evento.uaUsuario= "";
    this.evento.crFecha=  "";
    this.evento.crUsuario= "";

  this.theService.addRecord(this.evento)
      .subscribe((response: Response) => {
        if(response.status=="ok"){
          this.notificationService.success('Evento creado');
        }
        if(response.status=="error"){
          this.notificationService.error('Error, el Evento no pudo ser creado');
        }
    });
