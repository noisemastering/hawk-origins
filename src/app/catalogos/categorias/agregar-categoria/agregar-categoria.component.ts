...
export class AgregarCategoriaComponent implements OnInit {
  ...
  constructor(
    ...
    public dRef: MatDialogRef<AgregarCategoriaComponent> // Adaptar al objeto correspondiente
  ) { }
  ...
  duplicate(valor: string){ // Adaptar a los campos utilizados
    ...
    switch(valor){
      case 'nombre':
        this.duplicateService.checkForDuplicates("duplicate", "Descripcion", "categoriaes", this.theForm.controls['descripcion'].value, "Descripcion")
      ...
      break;
      case 'locacion':
        this.duplicateService.checkForDuplicates("duplicate", "RazonSocial", "categoriaes", this.theForm.controls['razon'].value, "RazonSocial")
        .subscribe((response: SimpleResponse) => {
          if(response.value=="Duplicate"){
  ...
  addRecord(): void {
    ...
    this.categoria.ID= '';
    this.categoria.Descripcion=  this.theForm.controls['descripcion'].value;
    this.categoria.Notas=  this.theForm.controls['notas'].value;
    this.categoria.Keywords=  JSON.stringify(this.keywords);
    this.categoria.uaFecha=  "";
    this.categoria.uaUsuario= "";
    this.categoria.crFecha=  "";
    this.categoria.crUsuario= "";

    this.theService.serviceURL= this.categoria.serviceURL;
  this.theService.addRecord(this.categoria)
      .subscribe((response: Response) => {
        if(response.status=="ok"){
          this.notificationService.success('Categoría creada');
        }
        if(response.status=="error"){
          this.notificationService.error('Error, la categoría no pudo ser creado');
        }
    });
